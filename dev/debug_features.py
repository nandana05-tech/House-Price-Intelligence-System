...
# Check feature names between the model and the input DataFrame
...
# Check the DataFrame generated by the feature engineering step
...
# Compare predictions using the original column order vs the reordered columns
print("\n=== Prediksi dengan X langsung ===")
pred1 = float(np.expm1(float(models.model_low.predict(X)[0])))
print(f"  Harga (X langsung):  Rp {pred1:,.0f}")

# Try reordering to match model_features if there are differences
if df_cols != model_features:
